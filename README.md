...
**可复用的写作规律**写进 `作者偏好档案.md`，驱动下一轮收敛。原文另存，**不进写稿上下文**。
整个 dsh 插件生态里，做"让 AI 写出合格稿"的很多，做"让 AI 写出像你的稿"的只有这一条路。

> **口径说明（不说大话）**：这句"原文不进写稿上下文"是准确的，它指的是——
> 写稿/写候选/要方向时，模型只拿到写作包，而包里除了**上一章结尾（≤900 字，接续用，包内注明）**
> 之外没有任何正文原文；标注原文、证据摘录、批注都另存为文件，写稿一律不读。
> 但有三处**作者亲手点下的按钮**会触发一次辅助调用并带上原文，各自有硬上限：
> ①「提炼规律」把标过的段落（≤40 段 × 240 字、总计 ≤20KB）交给模型压成规律；
> ②「补齐来源」把证据摘录里的引文（每条裁 160 字）与规律一起交给模型判断对应关系；
> ③「按批注微调」把**被批注的段落**（每段裁 800 字）连同前后段各 80 字交给模型改写。
> 除这三处，没有任何路径把正文原文送进模型。
...
**口味是怎么进去的**：写方向与写候选用的输入都只有**写作包**，而写作包的第 ② 节就是
`作者偏好档案`（只有规律、已档掉工作台的账目自动块）。写候选时还会把它念两遍：
系统提示点名"第 ② 节里『避免的写法』是作者明确否决过的，一条都不要出现"，
用户提示的**末尾**再把这份否决清单重述一次——写作包有三四千字，规律放中间容易被后文冲淡，
末尾重申一遍只花两三百字。
...
点「按批注微调」时才把**被批注的那几段**（每段裁到 800 字）连同批注、以及前后段各 80 字（供模型贴着邻段语气改）交给模型；
其余段落它碰不到。分两道防线：
...
0.2.0 定版前，我请三个独立视角把宿主半区、浏览器半区、"原文不进上下文"这句承诺各查了一遍，
所有高危项都**先复现再修**，并且把复现脚本固化成了回归测试（`test/workbench.test.mjs` 里的
「发布前审查的回归」两节）。修掉的四个高危，正好都是"会损坏作者数据"这一类：

| # | 问题 | 后果 | 现在的行为 |
|---|---|---|---|
| H1 | `readJsonBody` 按字符串拼接分片 | 多字节汉字被切在 chunk 边界 → 正文里**静默混进替换符**（实测 4 万字每份 2 个坏字） | 收 Buffer、`Buffer.concat` 后一次解码；回归测试用 65537 字节的错位分片验证 |
| H2 | 标注是"读-改-写"且落盘非原子 | 按住 `G` 连标时**标注互相覆盖**（实测并发 5 段只活 1 段）；断电留半截 JSON 会被当成空文件清零 | 读-改-写整段进串行队列 + 临时文件 rename 原子写；回归测试并发标 8 段要求一条不少 |
| H3 | `composeProfile` 见 `# 作者偏好档案` 开头就整份重建 | **手写档案被换成空骨架**，规律全丢（README 示例恰好就是这个开头） | 只认"旧版自动生成"标记；且档案每次被改写前都留一份 `.旧版.md` 备份 |
| H4 | `stage` 保存的章号没校验 | 传 `../../…` 可**写到作品目录之外**（全文件唯一越界写点） | 章号统一走 `1..10⁶` 校验；全部路由一致 |

同一轮还修了：微调凭据加正文指纹（作者在生成与采纳之间手改正文时**拒绝错位写回**）、
只结清真正进了本轮微调的批注（以前会把超过 20 条上限、没改过的批注也标成"已处理"）、
体检缓存核对作品（换书不再贴上一本的结论）、微调输入每段裁到 800 字（单换行排版整章成一段时不再退化成整章重写）、
以及浏览器半区 8 处"宿主少给一个字段就整屏崩"的读取点。

口径也按审查结果收窄了（见开头那段"口径说明"）：原来写"原文不进模型上下文"，
现在写清是**不进写稿上下文**，并明确列出三个作者亲手点下的按钮级例外与各自上限——
"补来源"会把证据摘录的引文交给模型这件事，之前的文案没写，属于说了大话。
