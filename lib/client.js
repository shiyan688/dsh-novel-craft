...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      wNcOpen: '开新章',
      wNcTitle: '开新章',
      wNcIntro: '抽卡解决"哪一段好"，这一步解决"候选稿从哪来"：先要一批结构不同的写法方向，再一篇一篇写出来，标完好段就合并成定稿。写候选用的输入只有写作包（规律 + 前情 + 台账 + 情节节点），不是全书原文。',
      wNcStepSetup: '这一章要写什么',
      wNcStepDirections: '方向',
      wNcStepDraft: '写候选',
      wNcStepMerge: '合并定稿',
      wNcChapter: '章号',
      wNcUseNext: '用下一章',
      wNcHasText: '这一章已有正文',
      wNcHasCandidates: '已有候选稿',
      wNcWhat: '本章设定（会进写作包第 ① 节，也是写候选的依据）',
      wNcFromOutline: '大纲里这一章的节点',
      wNcCount: '要几个方向',
      wNcCountHint: '建议 8–12 个：方向之间要换掉某个具体叙事决策，不是换词',
      wNcExtra: '本轮额外要求（可留空）',
      wNcExtraHint: '例如：这一章必须出现当铺掌柜；不要写超过两场戏；结尾要留钩子',
      wNcThinking: '模型正在想方向…',
      wNcMakeDirections: '让模型给方向',
      wNcDirectionsHint: '这些方向就是这一批候选的"卡池"。可以改名字、改说明、去掉不想要的——勾选哪些就写哪些。',
      wNcWriteCandidates: '开始写候选',
      wNcRemakeDirections: '换一批方向',
      wNcBack: '上一步',
      wNcDrafting: '正在写',
      wNcDraftDone: '写完了',
      wNcDraftHint: '一次一篇地写，写完一篇就落盘一篇。中途可以停，已经写好的不会丢；单篇失败只影响那一篇。',
      wNcStop: '停一下',
      wNcGoCards: '去抽卡选段',
      wNcPickOne: '至少要留一个方向',
      wNcMergeHint: '下面是你在抽卡页标为 👍 的段落，按稿件顺序列出。跨稿衔接处会自动写成〔此处需过渡〕标记——工作台不会让模型替你补写。',
      wNcNoMerge: '还没有合并稿：先去抽卡标几段，再回来点「合并」',
      wNcPicks: '段选中',
      wNcGaps: '处缺口',
      wNcGapHint: '有缺口是正常的（跨稿取段本来接不上）：合并稿会把这些位置留成标记，接着用「批注式微调」一处一处处理。',
      wNcPickContent: '取用内容',
      wNcPickReason: '取用原因',
      wNcReasonPlaceholder: '为什么用这一段（会进记录）',
      wNcFinalText: '合并稿（可以直接改，改完再写进正文）',
      wNcWriteFinal: '写入正文',
      wNcWritten: '已写入正文',
      wNcRebuild: '重新合并',
      wNcRecord: '取用记录',
      wNcNoPicks: '还没有标为 👍 的段落：先去抽卡选段',
      wReviseRestored: '从磁盘读回上次生成的微调稿',
      wStageRecall: '读回上次起草',
      wStageRecalled: '已读回上次起草',
      wPlotNoData: '这本书还没有可画曲线的章节数据'
    }
    const en = {
      ...
      wLedger: 'Item & buff ledger',
      wLedgerItems: 'items',
      wNcOpen: 'New chapter',
      wNcTitle: 'New chapter',
      wNcIntro: 'Gacha answers “which passage is good”; this step answers “where do the candidates come from”. Ask for a batch of structurally different directions, write them one at a time, then merge the passages you marked into a final draft. The only input to drafting is the writing pack (rules + recap + ledger + plot nodes) — not the whole manuscript.',
      wNcStepSetup: 'What this chapter does',
      wNcStepDirections: 'Directions',
      wNcStepDraft: 'Draft',
      wNcStepMerge: 'Merge',
      wNcChapter: 'Chapter',
      wNcUseNext: 'Use next',
      wNcHasText: 'This chapter already has text',
      wNcHasCandidates: 'candidates exist',
      wNcWhat: 'Chapter brief (becomes section ① of the pack, and the basis for drafting)',
      wNcFromOutline: 'Outline node for this chapter',
      wNcCount: 'How many directions',
      wNcCountHint: '8–12 works well: each direction must change a concrete narrative decision, not the wording',
      wNcExtra: 'Extra requirements for this round (optional)',
      wNcExtraHint: 'e.g. the pawnshop owner must appear; keep it to two scenes; end on a hook',
      wNcThinking: 'Model is thinking…',
      wNcMakeDirections: 'Ask the model for directions',
      wNcDirectionsHint: 'These directions are the card pool for this batch. Rename, rewrite or delete any of them — only the ticked ones get written.',
      wNcWriteCandidates: 'Write the candidates',
      wNcRemakeDirections: 'Another batch',
      wNcBack: 'Back',
      wNcDrafting: 'Writing',
      wNcDraftDone: 'Done',
      wNcDraftHint: 'One at a time, saved the moment each finishes. You can stop midway; finished ones are kept, and a failure only affects that one piece.',
      wNcStop: 'Stop',
      wNcGoCards: 'Go pick passages',
      wNcPickOne: 'Keep at least one direction',
      wNcMergeHint: 'Below are the passages you marked 👍 in the gacha view, in manuscript order. Seams between different candidates become 〔needs transition〕 markers — the workbench will not let a model write those for you.',
      wNcNoMerge: 'No merged draft yet: mark some passages first, then merge',
      wNcPicks: 'passages',
      wNcGaps: 'seams',
      wNcGapHint: 'Seams are expected — passages taken from different candidates do not join by themselves. They stay as markers, and you handle them with note-driven revision.',
      wNcPickContent: 'Content',
      wNcPickReason: 'Why this one',
      wNcReasonPlaceholder: 'why this passage (goes into the record)',
      wNcFinalText: 'Merged draft (edit freely before writing it in)',
      wNcWriteFinal: 'Write into the manuscript',
      wNcWritten: 'Written into the manuscript',
      wNcRebuild: 'Merge again',
      wNcRecord: 'Pick record',
      wNcNoPicks: 'Nothing is marked 👍 yet: go pick passages first',
      wReviseRestored: 'restored from the last generated revision',
      wStageRecall: 'Recall last draft',
      wStageRecalled: 'Recalled last draft',
      wPlotNoData: 'No chapter data to plot yet'
    }
                ...
                ? React.createElement(
                    ...
                    prov.marks.length === 0
                      ...
                      : React.createElement(
                          'div',
                          null,
                          ...(Array.isArray(prov.marks) ? prov.marks : []).map((m, mi) =>
                            React.createElement(
                              'div',
    ...
    function CraftWorkbench(props) {
      ...
      // 取用原因：标 👍 的段落顺手写一句"为什么用这一段"，合并时自动进「取用段落表」
      const [reasonAt, setReasonAt] = useState(-1)
      ...
      const fetchState = useCallback(async () => {
        ...
        // 插件被停用（宿主 503）或路由没注册时，别把它显示成"这个目录下没有候选稿"
        if (!r.ok) {
          const body = await r.json().catch(() => null)
          throw new Error(body !== null && body.error !== undefined ? String(body.error) : 'HTTP ' + String(r.status))
        }
        return await r.json()
      }, [])
      ...
      // 换篇就把取用原因的输入框收起来：否则 A 稿第 3 段写到一半换到 B 稿，
      // B 稿第 3 段下面会浮出带旧文本的输入框，一按回车就把原因记到了 B 上。
      useEffect(() => {
        setReasonAt(-1)
      }, [active, dir])
      ...
      const applyDir = useCallback(
        async (path) => {
          ...
          setDir(path)
          setState(null) // 先清空：否则 reload 完成前卡片区还显示上一本书的候选稿
          // 换了目录就是换了作品：凡是"属于上一本"的临时状态全部清掉。
          // 否则旧书的规律审阅块会把规律采纳进新书、旧书的合并稿会被写进新书的一章。
          setWs(null)
          setChapter(null)
          setWizard(false)
          setReasonAt(-1)
      ...
      /**
       * 标记串行化：按住 G 时一秒会发几十个请求，每个响应都把整个 marks 快照写回 state。
       * 乱序返回时界面会跳回旧快照，作者看到"刚标的没粘住"。排成一条链，行为可预期。
       */
      const markChainRef = useRef(Promise.resolve())

      const mark = (file, index, value, options) => {
        // 排进串行链：连标时的每个请求都基于最新的快照，响应也不会乱序覆盖界面
        const run = async () => {
          ...
          try {
            const r = await fetch(API + 'marks', {
              ...
              // reason 只在作者写了的时候带上：不写就不动那份取用理由文件
              body: JSON.stringify(
                options !== undefined && typeof options.reason === 'string'
                  ? { file, index, mark: next, reason: options.reason }
                  : { file, index, mark: next },
              ),
            })
        ...
        const chained = markChainRef.current.then(run, run)
        markChainRef.current = chained.catch(() => {})
        return chained
      }
        ...
        React.createElement(
          ...
          tab === 'cards' || tab === 'profile'
            ...
            : wsError !== ''
              ...
              : tab === 'chapters'
                ? wizard === true
                  ? React.createElement(NewChapterWizard, {
                      t,
                      // 默认章号＝下一章：作者点「开新章」时十有八九就是写下一章
                      chapter: ws !== null && Array.isArray(ws.chapters) && ws.chapters.length > 0 ? Math.max(...ws.chapters.map((c) => c.chapter)) + 1 : 1,
                      ...
                      onGoCards: async (dir) => {
                        // 候选目录就是刚写出来的那个：顺手设成当前卡池并跳过去，作者不用记路径
                        setWizard(false)
                        setTab('cards')
                        await applyDir(dir)
                      },
              ...
              : current === undefined
                ...
                : React.createElement(
                    ...
                    ...current.segments.map((seg, i) => {
                      ...
                      const reasonOf = state !== null && state.reasons !== undefined && state.reasons !== null ? state.reasons[current.file + '#' + String(i)] : undefined
                      if (m === 'good') {
                        buttons.push(
                          React.createElement(
                            'button',
                            {
                              ...
                              onClick: (e) => {
                                e.stopPropagation()
                                setReasonAt(reasonAt === i ? -1 : i)
                                setReasonText(typeof reasonOf === 'string' ? reasonOf : '')
                              },
                              title: t('wNcPickReason'),
                              style: btn({
                                fontSize: 11,
                                padding: '1px 6px',
                                opacity: reasonOf === undefined ? 0.55 : 1,
                                background: reasonOf === undefined ? 'transparent' : C.accent,
                                borderColor: reasonOf === undefined ? C.border : C.accent,
                                color: reasonOf === undefined ? 'inherit' : '#fff',
                              }),
                            },
                            '✎',
                          ),
                      ...
                      return React.createElement(
                        ...
                        React.createElement(
                          ...
                          React.createElement('div', { style: { fontSize: 15.5, lineHeight: 1.95, whiteSpace: 'pre-wrap' } }, seg),
                          reasonAt === i
                            ? React.createElement(
                                'div',
                                {
                                  style: { display: 'flex', gap: 6, marginTop: 5, alignItems: 'center' },
                                  onClick: (e) => e.stopPropagation(),
                                },
                                React.createElement('input', {
                                  value: reasonText,
                                  autoFocus: true,
                                  placeholder: t('wNcReasonPlaceholder'),
                                  onChange: (e) => setReasonText(e.target.value),
                                  onKeyDown: (e) => {
                                    if (e.key === 'Enter') {
                                      void mark(current.file, i, 'good', { reason: reasonText, toggle: false })
                                      setReasonAt(-1)
                                    }
                                    if (e.key === 'Escape') setReasonAt(-1)
                                  },
                                  style: {
                                    flex: 1,
                                    minWidth: 0,
                                ...
                                React.createElement(
                                  'button',
                                  {
                                    ...
                                    style: btn({ fontSize: 11.5, padding: '2px 8px' }),
                                  },
                                  t('save'),
                                ),
                                React.createElement('button', { onClick: () => setReasonAt(-1), style: btn({ fontSize: 11.5, padding: '2px 8px' }) }, t('cancel')),
                              )
                            : typeof reasonOf === 'string' && reasonOf !== ''
                              ? React.createElement(
                                  'div',
                                  {
                                    onClick: () => {
                                      setReasonAt(i)
                                      setReasonText(reasonOf)
                                    },
                                    style: { fontSize: 11.5, color: C.accent, marginTop: 3, cursor: 'pointer' },
                                  },
                                  '✎ ' + t('wNcPickReason') + '：' + reasonOf,
                                )
                              : null,
                        ),
                        React.createElement(
    ...
    /** 方向序号 → A/B/C…（与宿主半区 core/draft.js 的 letterOf 保持一致）。 */
    const DRAFT_LETTER = (index) => {
      const n = Number(index)
      if (!Number.isInteger(n) || n < 0) return '?'
      return n < 26 ? String.fromCharCode(65 + n) : String.fromCharCode(65 + Math.floor(n / 26) - 1) + String.fromCharCode(65 + (n % 26))
    }
    ...
    function TensionChart(props) {
      ...
      if (series.length === 0)
        return React.createElement(Empty, null, props.checked === true ? t('wPlotNoData') : t('wPlotNever'))
      const W = 720
      const H = 170
      const padL = 26
      const padR = 12
      ...
      const y = (v) => padT + (H - padT - padB) * (1 - (safe(v) - 1) / 4)
    ...
    function ChapterBoard(props) {
      ...
      if (ws === null || ws.found !== true) {
        // 正在读的时候别喊"没认出作品根"：大书要读几秒，作者会以为识别失败去手动指定。
        return React.createElement(
          'div',
          { style: { flex: 1, overflow: 'auto', padding: '16px 18px' } },
          props.busy === true ? React.createElement(Empty, null, t('wBusy')) : null,
          props.busy === true ? null : React.createElement('div', { style: { fontWeight: 600, marginBottom: 6 } }, '🗂 ' + t('wNoProject')),
          props.busy === true ? null : React.createElement('div', { style: { color: C.sub, fontSize: 13, lineHeight: 1.9 } }, t('wNoProjectHint')),
          props.busy === true ? null : React.createElement(
            'div',
          ...
          props.busy === true ? null : React.createElement(
            ...
            React.createElement('button', { onClick: props.onPickDir, style: btn({ fontSize: 12.5 }) }, '📁 ' + t('wSetProject')),
            React.createElement('button', { onClick: props.onNewChapter, style: btn({ fontSize: 12.5 }) }, '✍️ ' + t('wNcOpen')),
          ),
        )
      }
        ...
        React.createElement(
          ...
          React.createElement(
            'button',
            {
              ...
              style: btn({ marginLeft: 'auto', background: C.accent, borderColor: C.accent, color: '#fff', fontSize: 12 }),
            },
            '✍️ ' + t('wNcOpen'),
          ),
          React.createElement('button', { onClick: props.onRefresh, style: btn({ fontSize: 12 }) }, t('wRefresh')),
        ),
            ...
            ...chapters.map((row) => {
              ...
              return React.createElement(
                ...
                React.createElement(
                  'td',
                  { style: cell },
                  (row.marks === undefined || row.marks === null ? 0 : row.marks.good + row.marks.bad) === 0
                    ? React.createElement('span', { style: { color: C.sub } }, t('wNone'))
                    : React.createElement(
                        Chip,
                        null,
                        '👍' + String(row.marks === undefined || row.marks === null ? 0 : row.marks.good) + ' 👎' + String(row.marks === undefined || row.marks === null ? 0 : row.marks.bad),
                      ),
                ),
                React.createElement(
                  'td',
                  { style: cell },
                  (row.annotations === undefined || row.annotations === null ? 0 : row.annotations.total) === 0
                    ? React.createElement('span', { style: { color: C.sub } }, t('wNone'))
                    : React.createElement(
                        Chip,
                ...
                React.createElement(
                  'td',
                  { style: cell },
                  row.pack !== undefined && row.pack !== null && row.pack.exists
                    ? React.createElement(Chip, { tone: TONE.info, title: row.pack.path }, formatBytes(row.pack.bytes === undefined ? 0 : row.pack.bytes))
                    : React.createElement('span', { style: { color: C.sub } }, t('wNone')),
                ),
                React.createElement(
                  'td',
                  { style: cell },
                  row.comment === undefined || row.comment === null || row.comment.score === null || row.comment.score === undefined
                    ? React.createElement('span', { style: { color: C.sub } }, t('wNone'))
    ...
    function PackPanel(props) {
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            Chip,
            { tone: pack.budget !== undefined && pack.budget !== null && pack.budget.ok === true ? TONE.ok : TONE.warn },
            formatChars(pack.chars === undefined ? 0 : pack.chars),
          ),
          React.createElement(
            'button',
        ...
        result === null || result === undefined
          ...
          : React.createElement(
              ...
              React.createElement(
                ...
                result.cached === true ? '　· ' + t('wReviseRestored') : '',
    ...
    function ChapterDetail(props) {
      ...
      const load = useCallback(async () => {
        ...
        try {
          ...
          if (loadSeqRef.current !== seq) return // 快速换章时，慢响应不许覆盖新章
          setDetail(data)
          ...
          setPack(null)
          setCursor(0)
          // 待采纳的微调稿在磁盘上：切页签回来要能看见它，而不是只剩一个"重新生成"按钮
          // （那是花过模型钱的东西，丢了等于白花）
          setRevise(
            data.pendingDiff === undefined || data.pendingDiff === null
              ? null
              : {
                  ok: data.pendingDiff.ok,
                  problems: data.pendingDiff.problems,
                  warnings: data.pendingDiff.warnings,
                  diff: data.pendingDiff.diff,
                  model: data.pendingDiff.model,
                  used: data.pendingDiff.used,
                  total: data.pendingDiff.used,
                  cached: true,
                  hint: data.pendingDiff.stale === true ? '这一章的正文在生成微调之后被改过：请重新点「按批注微调」。' : '',
                },
          )
        } catch (e) {
          if (loadSeqRef.current === seq) setError(String(e && e.message ? e.message : e))
        } finally {
      ...
      useEffect(() => {
        ...
        // 目录选择器开着的时候让位：否则在遮罩后面按 j/k/A 会动光标、开批注框
        if (props.pickerOpen === true) return undefined
        const onKey = (event) => {
          if (event.metaKey || event.ctrlKey || event.altKey) return
                              ...
                              ...here.map((a) =>
                                React.createElement(
                                  ...
                                  '💬 ' +
                                    (a.note === ''
                                      ? (() => {
                                          const list = Array.isArray(detail.annotationTypes) ? detail.annotationTypes : []
                                          const hit = list.find((x) => x.id === a.type)
                                          return hit === undefined ? a.type : hit.label
                                        })()
                                      : a.note),
                                ),
    ...
    function PlotView(props) {
      ...
      const plotPart = checks !== null && checks.plot !== null && typeof checks.plot === 'object' ? checks.plot : null
      const ledgerPart = checks !== null && checks.ledger !== null && typeof checks.ledger === 'object' ? checks.ledger : null
      // 形状设防：metrics 可能是 checks.plot.metrics，也可能是被拍平的 checks.plot
      // （历史上这两种都出现过；面板永远不该因为一个字段缺失就把工作台带崩）
      const metrics =
        plotPart === null
          ? null
          : plotPart.metrics !== null && typeof plotPart.metrics === 'object'
            ? plotPart.metrics
            : typeof plotPart.tension !== 'undefined'
              ? plotPart
              : null
      const findings = plotPart !== null && Array.isArray(plotPart.findings) ? plotPart.findings : []
      const ledgerFindings = ledgerPart !== null && Array.isArray(ledgerPart.findings) ? ledgerPart.findings : []
      const ledgerSummary = ledgerPart !== null && ledgerPart.summary !== null && typeof ledgerPart.summary === 'object' ? ledgerPart.summary : null
      const chapters = ws !== null && Array.isArray(ws.chapters) ? ws.chapters : []
      const series = metrics === null || !Array.isArray(metrics.tension) ? [] : metrics.tension
      const scores = chapters
        .filter((c) => c.comment !== undefined && c.comment !== null && c.comment.score !== null && c.comment.score !== undefined)
        .map((c) => ({ chapter: c.chapter, score: c.comment.score }))
      const byLevel = (a, b) =>
        (LEVEL_ORDER[a.level] === undefined ? 3 : LEVEL_ORDER[a.level]) -
        (LEVEL_ORDER[b.level] === undefined ? 3 : LEVEL_ORDER[b.level])
      const sorted = findings.slice().sort(byLevel)
      const ledgerSorted = ledgerFindings.slice().sort(byLevel)
      const jump = (chapter) => {
      ...
      return React.createElement(
        ...
        checks === null
          ...
          : React.createElement(TensionChart, { t, series, scores, checked: true, onPick: jump }),
        ...
        React.createElement(
          ...
          '🧾 ' +
            t('wLedger') +
            (ledgerSummary === null ? '' : ' · ' + String(ledgerSummary.items) + ' ' + t('wLedgerItems')) +
            (ledgerSorted.length === 0 ? '' : ' · ' + String(ledgerSorted.length) + ' ' + t('wIssues')),
        ),
        checks === null
          ? React.createElement(Empty, null, t('wPlotNever'))
          : ledgerSorted.length === 0
            ? React.createElement(Empty, null, t('wPlotNone'))
            : React.createElement(
                'div',
                null,
                ...ledgerSorted.map((f, i) =>
                  React.createElement(
                    'div',
                    {
                      key: 'l' + String(i),
                    ...
                    React.createElement(
                      ...
                      { style: { display: 'flex', alignItems: 'center', gap: 8, fontSize: 11.5, color: C.sub } },
                      React.createElement(Chip, { tone: TONE[f.level] === undefined ? C.sub : TONE[f.level] }, String(f.code)),
                      f.item === '' || f.item === undefined ? null : React.createElement('span', null, String(f.item)),
                      f.chapter === null || f.chapter === undefined
                        ? null
                        : React.createElement(
                            'span',
    ...
    function StageView(props) {
      ...
      /** 读回上次起草（切页签把界面草稿弄丢了时用；产物本来就归档在磁盘上）。 */
      const recall = async () => {
        setBusy(true)
        setNotice('')
        try {
          const data = await postJson('stage', { action: 'recall', stage: activeId })
          setDraft(data.text === undefined ? '' : data.text)
          setTarget(data.produce === undefined ? '' : data.produce)
          setNotice(t('wStageRecalled') + '：' + String(data.archive))
        } catch (e) {
          setNotice(String(e && e.message ? e.message : e))
        } finally {
          setBusy(false)
        ...
        React.createElement(
            ...
            ...stages.map((stage) =>
              React.createElement(
                ...
                React.createElement(
                  ...
                  ...(Array.isArray(stage.artifacts) ? stage.artifacts : []).map((a) =>
                    React.createElement('div', { key: a.path }, (a.ok === true ? '✓ ' : '· ') + a.label + '　' + a.path + (a.exists === true ? '（' + formatBytes(a.bytes) + '）' : '')),
                  ),
                ),
                ...(Array.isArray(stage.gate) ? stage.gate : []).filter((g) => g.ok !== true).map((g, i) =>
                  React.createElement(
                    'div',
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              React.createElement(
                'button',
                { onClick: () => void recall(), disabled: busy === true, style: btn({ fontSize: 12 }) },
                '↻ ' + t('wStageRecall'),
              ),
     ...
     *   ① 这一章要写什么（+ 要几个方向）
     *   ② 方向清单（可改可删可勾）
     *   ③ 一次一篇写候选（看得见进度，能中断、能重来某一篇）
     *   ④ 合并选中段落 → 写入正文（原稿自动备份）
     */
    function NewChapterWizard(props) {
      const t = props.t
      ...
      const [versionName, setVersionName] = useState('合并稿')
      ...
      const loadStatus = useCallback(
        async (n) => {
          ...
          setNotice('')
          try {
            const data = await postJson('newchapter', { action: 'status', chapter: n })
            setWstatus(data)
            setSetupText(data.setup === undefined ? '' : data.setup.text)
          } catch (e) {
            setNotice(String(e && e.message ? e.message : e))
          } finally {
            setBusy('')
      ...
      useEffect(() => {
        void loadStatus(chapter)
      }, [chapter, loadStatus])

      // 卸载即中止在途的"写候选"循环：切页签/关工作台都会卸载本组件，
      // 循环若不中止就会继续一篇篇烧模型，而且「停一下」按钮已经消失。
      useEffect(() => () => {
      ...
      const runDirections = async () => {
        ...
        setNotice('')
        try {
          if (setupText.trim() !== '') await postJson('setup', { chapter, text: setupText })
          const data = await postJson('newchapter', { action: 'directions', chapter, extra, count })
          setDirections(data.directions)
          const next = {}
          for (const d of data.directions) next[d.index] = true
          setPicked(next)
          setStep('directions')
          if (Array.isArray(data.packWarnings)) for (const w of data.packWarnings) setNotice(String(w))
        } catch (e) {
          setNotice(String(e && e.message ? e.message : e))
        } finally {
          setBusy('')
      ...
      const runDrafts = async () => {
        const list = directions.filter((d) => picked[d.index] !== false)
        if (list.length === 0) {
          setNotice(t('wNcPickOne'))
          return
        }
        ...
        for (let i = 0; i < list.length; i += 1) {
          ...
          const index = directions.indexOf(list[i])
          setProgress({ done: i, total: list.length, current: list[i].name })
          try {
            const data = await postJson('newchapter', {
              action: 'draft',
              chapter,
              index,
              direction: list[i],
            ...
            setResults((prev) => [...prev, { ...data, name: list[i].name }])
            if (data.warning !== undefined && data.warning !== '') setNotice(String(data.warning))
          } catch (e) {
            setResults((prev) => [...prev, { file: '', name: list[i].name, error: String(e && e.message ? e.message : e), chars: 0 }])
          }
        }
        setProgress((p) => ({ ...p, done: list.length, current: '' }))
        void loadStatus(chapter)
      }

      const toCards = async () => {
        if (wstatus !== null && wstatus.candidatesDir !== undefined) props.onGoCards(wstatus.candidatesDir)
      }

      const buildMerge = async (picks) => {
        setBusy('merge')
        setNotice('')
        try {
          const data = await postJson('newchapter', { action: 'merge', chapter, picks, versionName })
          setMergeInfo(data)
      ...
      const refreshPicks = async () => {
        ...
        setNotice('')
        try {
          const data = await postJson('newchapter', { action: 'status', chapter })
          setWstatus(data)
          if (Array.isArray(data.picks) && data.picks.length > 0) await buildMerge(data.picks)
          else setNotice(t('wNcNoPicks'))
        } catch (e) {
          setNotice(String(e && e.message ? e.message : e))
        } finally {
          setBusy('')
      ...
      const movePick = async (picks, i, delta) => {
        const next = [...picks]
        const j = i + delta
        ...
        next[i] = next[j]
        next[j] = tmp
        await buildMerge(next)
      }

      const setReason = async (picks, i, reason) => {
        const next = picks.map((p, k) => (k === i ? { ...p, reason } : p))
        // 只改这一行：曾经漏了 k === i，结果给第 3 段写原因时整张表都显示同一句
        // （纯显示错数据），而且 picks 没同步，重排/重新合并后写的原因还会消失。
        setMergeInfo({
          ...mergeInfo,
          picks: next,
          rows: (mergeInfo.rows === undefined ? [] : mergeInfo.rows).map((row, k) => (k === i ? { ...row, reason } : row)),
        })
        try {
          await postJson('marks', { file: picks[i].file, index: picks[i].index, mark: 'good', reason })
        } catch (e) {
          setNotice(String(e && e.message ? e.message : e))
        }
      }
      ...
      const writeFinal = async () => {
        ...
        setNotice('')
        try {
          const data = await postJson('newchapter', { action: 'finalize', chapter, text: finalText })
          setNotice(t('wNcWritten') + '：' + String(data.path) + (data.backupPath === '' ? '' : '（原稿已备份）'))
          props.onDone()
        } catch (e) {
          setNotice(String(e && e.message ? e.message : e))
        } finally {
        ...
        React.createElement(
          'div',
          {
            style: {
              ...
              padding: '3px 9px',
              borderRadius: 999,
              border: '1px solid ' + (step === id ? C.accent : C.border),
              color: step === id ? '#fff' : C.sub,
              background: step === id ? C.accent : 'transparent',
              whiteSpace: 'nowrap',
            },
          },
          label,
      ...
      const box = { background: C.card, border: '1px solid ' + C.border, borderRadius: 10, padding: 12, marginBottom: 10 }
      const label = { fontSize: 12, color: C.sub, marginBottom: 5 }
      const area = {
        width: '100%',
        boxSizing: 'border-box',
        background: C.bg,
      ...
      const stepBar = React.createElement(
        'div',
        { style: { display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', marginBottom: 10 } },
        stepBtn('setup', '① ' + t('wNcStepSetup')),
        stepBtn('directions', '② ' + t('wNcStepDirections')),
        stepBtn('draft', '③ ' + t('wNcStepDraft')),
        stepBtn('merge', '④ ' + t('wNcStepMerge')),
      )

      const noticeLine =
        notice === ''
          ? null
          : React.createElement('div', { style: { fontSize: 12, color: TONE.warn, margin: '6px 0', lineHeight: 1.7 } }, notice)

      const setupStep = React.createElement(
        'div',
        null,
        React.createElement('div', { style: box },
          React.createElement('div', { style: label }, t('wNcChapter')),
          React.createElement(
            'div',
            { style: { display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' } },
            React.createElement('input', {
              ...
              min: 1,
              onChange: (e) => {
                // 清空输入框时 Number('') = 0，会让宿主报"chapter 必须是从 1 开始的整数"；
                // 只在解析出正整数时才更新，剩下的交给输入框自己显示。
                const n = Number(e.target.value)
                if (Number.isInteger(n) && n >= 1) setChapter(n)
              },
              style: { ...area, width: 90, resize: 'none' },
            }),
            wstatus !== null && wstatus.suggestedChapter !== undefined
              ? React.createElement('button', { onClick: () => setChapter(wstatus.suggestedChapter), style: btn({ fontSize: 12 }) }, t('wNcUseNext') + ' ' + String(wstatus.suggestedChapter))
              : null,
            wstatus !== null && wstatus.hasText === true ? React.createElement(Chip, { tone: TONE.warn }, t('wNcHasText') + ' ' + formatChars(wstatus.chars)) : null,
            wstatus !== null && wstatus.candidateCount > 0 ? React.createElement(Chip, { tone: TONE.info }, t('wNcHasCandidates') + ' ' + String(wstatus.candidateCount)) : null,
          ),
        ),
        React.createElement('div', { style: box },
          React.createElement('div', { style: label }, t('wNcWhat')),
          wstatus !== null && wstatus.outlineNode !== null && wstatus.outlineNode !== undefined
            ? React.createElement(
                'div',
                { style: { fontSize: 12, color: C.sub, marginBottom: 6, lineHeight: 1.7 } },
                t('wNcFromOutline') + '：' + [wstatus.outlineNode.goal, wstatus.outlineNode.conflict, wstatus.outlineNode.turn].filter((x) => x !== '' && x !== undefined).join('　/　'),
              )
            : null,
          React.createElement('textarea', { value: setupText, onChange: (e) => setSetupText(e.target.value), rows: 8, style: area }),
        ),
        React.createElement('div', { style: box },
          React.createElement('div', { style: label }, t('wNcCount')),
          React.createElement(
            'div',
            { style: { display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' } },
            React.createElement('input', { type: 'number', value: count, min: 3, max: 16, onChange: (e) => setCount(Number(e.target.value)), style: { ...area, width: 90, resize: 'none' } }),
            React.createElement('span', { style: { fontSize: 11.5, color: C.sub } }, t('wNcCountHint')),
          ),
          React.createElement('div', { style: { ...label, marginTop: 10 } }, t('wNcExtra')),
          React.createElement('textarea', { value: extra, onChange: (e) => setExtra(e.target.value), rows: 3, placeholder: t('wNcExtraHint'), style: area }),
        ),
        React.createElement(
          'button',
          { onClick: () => void runDirections(), disabled: busy !== '', style: btn({ background: C.accent, borderColor: C.accent, color: '#fff', fontSize: 12.5 }) },
          busy === 'directions' ? t('wNcThinking') : '✨ ' + t('wNcMakeDirections'),
        ),
        noticeLine,
      )

      const directionsStep = React.createElement(
        'div',
        null,
        React.createElement('div', { style: { color: C.sub, fontSize: 12, lineHeight: 1.8, marginBottom: 8 } }, t('wNcDirectionsHint')),
        ...directions.map((d, i) =>
          React.createElement(
            'div',
            { key: 'd' + String(i), style: { ...box, display: 'flex', gap: 9, alignItems: 'flex-start' } },
            React.createElement('input', {
              type: 'checkbox',
              checked: picked[d.index] !== false,
              onChange: (e) => setPicked({ ...picked, [d.index]: e.target.checked }),
              style: { marginTop: 3 },
            }),
            React.createElement(
              'div',
              ...
              React.createElement(
                ...
                React.createElement(Chip, { tone: C.accent }, DRAFT_LETTER(d.index)),
                React.createElement('input', {
                  value: d.name,
                  onChange: (e) => setDirections(directions.map((x, k) => (k === i ? { ...x, name: e.target.value } : x))),
                  style: { ...area, flex: 1, resize: 'none', padding: '4px 7px' },
                }),
              ),
              React.createElement('textarea', {
                value: d.detail,
                rows: 2,
                onChange: (e) => setDirections(directions.map((x, k) => (k === i ? { ...x, detail: e.target.value } : x))),
                style: { ...area, marginTop: 6 },
              }),
            ),
            React.createElement(
              'button',
              { onClick: () => setDirections(directions.filter((x, k) => k !== i)), style: btn({ padding: '2px 7px', fontSize: 12 }) },
              '✕',
            ),
          ),
        ),
        React.createElement(
          ...
          React.createElement(
            'button',
            { onClick: () => void runDrafts(), disabled: busy !== '', style: btn({ background: C.accent, borderColor: C.accent, color: '#fff', fontSize: 12.5 }) },
            '✍️ ' + t('wNcWriteCandidates') + '（' + String(directions.filter((d) => picked[d.index] !== false).length) + '）',
          ),
          React.createElement('button', { onClick: () => void runDirections(), disabled: busy !== '', style: btn({ fontSize: 12.5 }) }, '↻ ' + t('wNcRemakeDirections')),
          React.createElement('button', { onClick: () => setStep('setup'), style: btn({ fontSize: 12.5 }) }, '← ' + t('wNcBack')),
        ),
        noticeLine,
      )

      const draftStep = React.createElement(
        'div',
        null,
        React.createElement(
          'div',
          ...
          React.createElement(
            ...
            progress.current === ''
              ? t('wNcDraftDone') + '（' + String(results.length) + '/' + String(progress.total) + '）'
              : t('wNcDrafting') + ' ' + String(progress.done + 1) + '/' + String(progress.total) + ' · ' + progress.current,
          ),
          React.createElement('div', { style: { fontSize: 11.5, color: C.sub, marginTop: 5, lineHeight: 1.7 } }, t('wNcDraftHint')),
          progress.current !== ''
            ? React.createElement('button', { onClick: () => (abortRef.current = true), style: btn({ marginTop: 8, fontSize: 12 }) }, t('wNcStop'))
            : null,
        ),
        ...results.map((r, i) =>
          React.createElement(
            'div',
            { key: 'r' + String(i), style: { ...box, display: 'flex', gap: 8, alignItems: 'center' } },
            React.createElement(Chip, { tone: r.error === undefined ? TONE.ok : TONE.error }, r.error === undefined ? '✓' : '✗'),
            React.createElement('span', { style: { fontSize: 12.5, flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, String(r.name)),
            r.error === undefined
              ? React.createElement('span', { style: { fontSize: 11.5, color: C.sub } }, formatChars(r.chars === undefined ? 0 : r.chars) + ' · ' + String(r.file))
              : React.createElement('span', { style: { fontSize: 11.5, color: TONE.error } }, String(r.error).slice(0, 60)),
          ),
        ),
        React.createElement(
          'div',
          ...
          React.createElement(
            'button',
            {
              onClick: () => void toCards(),
              disabled: wstatus === null || results.filter((r) => r.error === undefined).length === 0,
              style: btn({ background: C.accent, borderColor: C.accent, color: '#fff', fontSize: 12.5 }),
            },
            '🎴 ' + t('wNcGoCards'),
          ),
          React.createElement('button', { onClick: () => void refreshPicks(), disabled: busy !== '', style: btn({ fontSize: 12.5 }) }, '④ ' + t('wNcStepMerge')),
          React.createElement('button', { onClick: () => setStep('directions'), style: btn({ fontSize: 12.5 }) }, '← ' + t('wNcBack')),
        ),
        noticeLine,
      )

      const rows = mergeInfo === null || !Array.isArray(mergeInfo.rows) ? [] : mergeInfo.rows
      const picks = mergeInfo === null || !Array.isArray(mergeInfo.picks) ? [] : mergeInfo.picks
      const mergeStep = React.createElement(
        'div',
        null,
        React.createElement('div', { style: { color: C.sub, fontSize: 12, lineHeight: 1.8, marginBottom: 8 } }, t('wNcMergeHint')),
        mergeInfo === null
          ? React.createElement(Empty, null, t('wNcNoMerge'))
          : React.createElement(
              'div',
              null,
              React.createElement(
                ...
                React.createElement(
                  'div',
                  { style: { display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' } },
                  React.createElement(Chip, { tone: TONE.info }, String(rows.length) + ' ' + t('wNcPicks')),
                  React.createElement(Chip, { tone: mergeInfo.gaps > 0 ? TONE.warn : TONE.ok }, String(mergeInfo.gaps) + ' ' + t('wNcGaps')),
                  React.createElement(Chip, null, formatChars(mergeInfo.chars)),
                  React.createElement('input', {
                    value: versionName,
                    onChange: (e) => setVersionName(e.target.value),
                    style: { ...area, width: 160, resize: 'none', padding: '4px 7px' },
                  }),
                  React.createElement('button', { onClick: () => void refreshPicks(), style: btn({ fontSize: 12 }) }, '↻ ' + t('wRefresh')),
                ),
              ),
              React.createElement(
                'table',
                ...
                React.createElement(
                  ...
                  React.createElement(
                    'tr',
                    null,
                    ...['#', t('wPackSource'), t('wNcPickContent'), t('wNcPickReason'), t('wPackChars'), ''].map((h, i) =>
                      React.createElement(
                        'th',
                        { key: 'h' + String(i), style: { textAlign: 'left', padding: '5px 6px', fontSize: 11, color: C.sub, fontWeight: 500, borderBottom: '1px solid ' + C.border } },
                        h,
                      ),
                    ),
                  ),
                ...
                React.createElement(
                  ...
                  ...rows.map((row, i) =>
                    React.createElement(
                      'tr',
                      { key: 'p' + String(i) },
                      React.createElement('td', { style: { padding: '5px 6px', borderBottom: '1px solid ' + C.border, color: C.sub } }, String(i + 1)),
                      React.createElement('td', { style: { padding: '5px 6px', borderBottom: '1px solid ' + C.border, whiteSpace: 'nowrap' } }, row.source),
                      React.createElement('td', { style: { padding: '5px 6px', borderBottom: '1px solid ' + C.border, color: C.sub } }, String(row.preview).slice(0, 34)),
                      React.createElement(
                        'td',
                        { style: { padding: '5px 6px', borderBottom: '1px solid ' + C.border } },
                        React.createElement('input', {
                          value: row.reason === undefined ? '' : row.reason,
                          placeholder: t('wNcReasonPlaceholder'),
                          onChange: (e) => void setReason(picks, i, e.target.value),
                          style: { ...area, resize: 'none', padding: '3px 6px' },
                        }),
                      ),
                      React.createElement('td', { style: { padding: '5px 6px', borderBottom: '1px solid ' + C.border, color: C.sub } }, String(row.chars)),
                      React.createElement(
                        'td',
                        { style: { padding: '5px 6px', borderBottom: '1px solid ' + C.border, whiteSpace: 'nowrap' } },
                        React.createElement('span', { onClick: () => void movePick(picks, i, -1), style: { cursor: 'pointer', color: C.sub, marginRight: 6 } }, '↑'),
                        React.createElement('span', { onClick: () => void movePick(picks, i, 1), style: { cursor: 'pointer', color: C.sub } }, '↓'),
                      ),
                    ),
                  ),
                ),
              ...
              mergeInfo.gaps > 0
                ? React.createElement('div', { style: { fontSize: 12, color: TONE.warn, margin: '8px 0', lineHeight: 1.7 } }, '⚠️ ' + t('wNcGapHint'))
                : null,
              React.createElement('div', { style: { ...label, marginTop: 10 } }, t('wNcFinalText')),
              React.createElement('textarea', { value: finalText, onChange: (e) => setFinalText(e.target.value), rows: 14, style: area }),
            ),
        React.createElement(
          'div',
          ...
          React.createElement(
            'button',
            { onClick: () => void writeFinal(), disabled: busy !== '' || finalText.trim() === '', style: btn({ background: finalText.trim() === '' ? 'transparent' : TONE.ok, borderColor: finalText.trim() === '' ? C.border : TONE.ok, color: finalText.trim() === '' ? C.sub : '#fff', fontSize: 12.5 }) },
            busy === 'final' ? '…' : '✅ ' + t('wNcWriteFinal'),
          ),
          React.createElement('button', { onClick: () => void refreshPicks(), disabled: busy !== '', style: btn({ fontSize: 12.5 }) }, '↻ ' + t('wNcRebuild')),
          mergeInfo !== null && mergeInfo.mergeDocPath !== undefined
            ? React.createElement('span', { style: { fontSize: 11.5, color: C.sub, alignSelf: 'center' } }, t('wNcRecord') + '：' + String(mergeInfo.mergeDocPath))
            : null,
        ),
        noticeLine,
      )
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          { style: { display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' } },
          React.createElement('button', { onClick: props.onBack, style: btn({ fontSize: 12 }) }, t('wBack')),
          React.createElement('div', { style: { fontWeight: 700, fontSize: 15 } }, '✍️ ' + t('wNcTitle')),
        ),
        React.createElement('div', { style: { color: C.sub, fontSize: 12, margin: '6px 0 12px', lineHeight: 1.85 } }, t('wNcIntro')),
        stepBar,
        step === 'setup' ? setupStep : null,
        step === 'directions' ? directionsStep : null,
        step === 'draft' ? draftStep : null,
        step === 'merge' ? mergeStep : null,
      )
    }
