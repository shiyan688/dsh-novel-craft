  ...
  factory: (require) => {
    ...
    function NewChapterWizard(props) {
      ...
      const setReason = async (picks, i, reason) => {
        ...
        setMergeInfo({ ...mergeInfo, rows: next.map((p, k) => ({ ...mergeInfo.rows[k], reason })) })
        try {
      ...
      const draftStep = React.createElement(
        ...
        ...results.map((r, i) =>
          React.createElement(
            ...
            r.error === undefined
              ? React.createElement('span', { style: { fontSize: 11.5, color: C.sub } }, formatChars(r.chars) + ' · ' + String(r.file))
        ...
        React.createElement(
          ...
          React.createElement(
            'button',
            { onClick: () => void toCards(), disabled: results.filter((r) => r.error === undefined).length === 0, style: btn({ background: C.accent, borderColor: C.accent, color: '#fff', fontSize: 12.5 }) },
            '🎴 ' + t('wNcGoCards'),
          ),
