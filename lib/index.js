 ...
 * - `marks.json`      机器可读的好/坏标注，写稿永不读（只有作者点「提炼」时以编号形式进一次辅助调用）；
 * - `证据摘录.md`      作者标注的**原文**，给人回查、给提炼/补来源当输入，**不要读进写稿上下文**；
...
async function writeReasons(dir, reasons) {
  return writeAtomic(join(stateDirOf(dir), REASONS_FILE), JSON.stringify(reasons, null, 2))
}

async function writeMarks(dir, marks) {
  return writeAtomic(marksPathOf(dir), JSON.stringify(marks, null, 2))
}

/** 列出候选目录下的文本候选稿并切段。
 *  有上限：候选目录一旦被误设成作品根（几百章正文），没有上限的话每次 state
 *  都会把全书读进内存再整个塞进 JSON 响应。
 */
const CANDIDATE_LIMIT = { files: 60, bytesPerFile: 512 * 1024 }
...
async function readCandidates(dir) {
  ...
  const files = names
    ...
    .sort()
    // 一次最多读这么多篇：候选目录被误设成作品根时（几百章正文），
    // 没有上限的话每次打开抽卡页都会把全书读进内存、再整个塞进 JSON 响应。
    const limited = files.length > CANDIDATE_LIMIT.files ? files.slice(0, CANDIDATE_LIMIT.files) : files
  const out = []
  for (const file of limited) {
    try {
      const info = await stat(join(dir, file))
      if (info.size > CANDIDATE_LIMIT.bytesPerFile) continue // 超大文件基本不是候选稿
...
async function writeDistillState(dir, state) {
  return writeAtomic(distillPathOf(dir), JSON.stringify(state, null, 2))
}
...
function composeProfile({ stats, existing, dir }) {
  ...
  // 只有两种情况会给骨架：档案是空的，或者它本来就是旧版自动生成的（带 GENERATED_HINT）。
  // 这里曾经多一个 `startsWith('# 作者偏好档案')` 的条件——README 给的手写示例开头恰好就是
  // 这六个字，结果作者手写的档案一点「收录证据」就被换成空骨架，规律全部消失（审查实测）。
  // 手写档案无论开头是什么，都只追加自动块，一个字都不动。
  if (existing.trim() === '' || existing.includes(GENERATED_HINT)) {
...
/** 章号边界：1 起、上限 10^6。曾经只查"是整数"，chapter=0/-3/999999999 会写出
 *  批注/第-3章.json 这类垃圾文件；统一从这里过，行为一致。 */
const chapterArg = (value, label = 'chapter') => {
  const n = Number(value)
  if (!Number.isInteger(n) || n < 1 || n > 1e6) throw new Error(`${label} 必须是从 1 开始的整数（上限 1000000）`)
  return n
}
 ...
 *
 * 必须收 Buffer、concat 之后一次 toString('utf8')：按字符串拼接的话，一个多字节的
 * 中文字符正好被切在两个 chunk 的边界上时会被拆成替换符——正文里悄悄混进 ""。
 * 发布前的审查在 4 万字正文里实测到每份 2 个坏字，而备份的是旧稿、坏字已进新稿，
 * 作者肉眼几乎不可能发现。这是整个插件最不能出的一类事故，所以这里值得写这么长。
 */
  ...
  return new Promise((resolve) => {
    ...
    req.on('end', () => {
      ...
      try {
        ...
        finish({ __error: '请求体不是合法 JSON' })
      }
    })
    req.on('error', () => finish({ __error: '读取请求体失败' }))
  })
}
...
export async function apply(ctx) {
   ...
   * 写证据摘录 + 刷新档案自动块。
   *
   * 档案在**每次**被工作台改写前都先留一份 `作者偏好档案.旧版.md`——
   * 以前只备份"旧版自动生成"那一种，手写档案不留底；一旦哪里的判断错了，
   * 作者的规律就无处可寻。这种文件多一个没有坏处。
   */
  const refreshArtifacts = async (dir, data) => {
    await mkdir(stateDirOf(dir), { recursive: true })
    const now = new Date().toISOString()
    if (data.profile.text.trim() !== '') {
  ...
  route('marks', async (body) => {
    ...
    // 读-改-写必须在**同一次队列任务**里完成：只把写排进队列的话，两个并发请求
    // 会读到同一份旧快照，后写的把前一个标注覆盖掉（审查实测：并发标 5 段只活 1 段）。
    const withReason = typeof body.reason === 'string'
    await updateJsonFile(marksPathOf(dir), (marks) => {
    ...
    if (withReason) {
      ...
      await updateJsonFile(join(stateDirOf(dir), REASONS_FILE), (reasons) => {
        if (body.reason.trim() === '') delete reasons[key]
        else reasons[key] = clip(body.reason.trim(), 200)
        return reasons
      })
    }
    return { marks: await readMarks(dir), reasons: await readReasons(dir) }
  })
  ...
  route('project', async (body) => {
    ...
    if (action === 'set') {
      ...
      // 必须是目录：传文件路径也放行的话，看板会"认出来了"但一片空白，
      // 后面的写入再报一条生硬的 ENOTDIR，作者不知道哪错了。
      let isDir = false
      try {
        isDir = (await stat(dir)).isDirectory()
      } catch {
        throw new Error('这个目录不存在：' + dir)
      }
      if (!isDir) throw new Error('这是一个文件，不是目录：' + dir)
  ...
  route('workspace', async (body) => {
    ...
    // 非深检只能用**同一作品**的缓存结论：换了作品根还拿旧缓存，上一本的体检
    // 会按章号贴到新书的徽标上（曾经就是这么干的）。
    const cacheKey = project.projectDir + '|' + ws.chapters.length
    const checks =
      body.deep === true
        ? await runChecks(project.projectDir, ws.chapters, false)
        : checkCache.key === cacheKey
          ? checkCache.value
          : null
  ...
  route('chapter', async (body) => {
    ...
    const chapter = chapterArg(body.chapter)
    ...
    return {
      ...
      pendingDiff: await (async () => {
        const pending = await readJsonSafe(join(W.stateDirOf(project.projectDir), '微调', `第${chapter}章 待采纳.json`))
        if (pending === null || typeof pending.revised !== 'object' || pending.revised === null) return null
        ...
        const stale = typeof pending.textHash === 'string' && pending.textHash !== ''
          ? createHash('sha1').update(detail.text).digest('hex').slice(0, 16) !== pending.textHash
          : false
        return {
          ...
          stale,
          ok: check.ok && !stale,
          problems: stale ? ['这一章的正文在生成微调之后被改过：请重新点「按批注微调」。'] : check.errors,
          warnings: check.warnings,
          diff: ANN.revisionDiff({ originalText: detail.text, revisedText: rebuilt, annotations }),
        }
      })(),
  ...
  route('revise', async (body) => {
    ...
    // 单换行排版的 txt：整章会被当成一段，"只改被批注段"就退化成"改整章"。
    // 这种情况如实提醒作者，别让它悄悄发生。
    const singleSegment = detail.segments.length === 1 && charCount(detail.text) > 1500
    ...
    // textHash 是当时正文的指纹：作者在生成与采纳之间手改正文（段号会漂移），
    // 旧改写就会嫁接到错误的段落上——verifyRevision 对的是"自己拼的自己"，拦不住这种错位。
    await writeJson(join(W.stateDirOf(project.projectDir), '微调', `第${chapter}章 待采纳.json`), {
      ...
      revised,
      textHash: createHash('sha1').update(detail.text).digest('hex').slice(0, 16),
    ...
    return {
      ...
      warnings: singleSegment
        ? ['这一章的正文在磁盘上没有空行分段（整章被当成一段）：微调等于让模型改写整章，建议先按段落整理空行，或只用它做整体润色。']
        : [],
    }
  })
  ...
  route('revise-apply', async (body) => {
    ...
    const detail = await W.readChapter(project.projectDir, chapter)
    if (!detail.found) throw new Error(`没有找到第${chapter}章的正文`)
    // 指纹核对：正文变了（哪怕只是手动改了一个字），这份待采纳凭据就作废——
    // 让作者重新微调，而不是把旧改写嫁接到漂移后的段落上。
    if (typeof pending.textHash === 'string' && pending.textHash !== '') {
      const now = createHash('sha1').update(detail.text).digest('hex').slice(0, 16)
      if (now !== pending.textHash) {
        throw new Error('这一章的正文在生成微调之后被改过（指纹对不上）：请重新点「按批注微调」，别把旧改写写错位置。')
      }
    }
    const indexes = Array.isArray(pending.indexes) ? pending.indexes : Object.keys(pending.revised).map(Number)
    const rebuilt = ANN.applyRevision(detail.text, pending.revised, indexes)
    ...
    const annotations = await ANN.readAnnotations(project.projectDir, chapter)
    // 只结清**进了本轮微调**的那批批注：一次微调最多带 20 条，剩下的还是待处理，
    // 不能一概标成"已处理"——那会让作者以为都改完了。
    const touched = new Set((Array.isArray(pending.indexes) ? pending.indexes : []).map(Number))
    let resolved = 0
    for (const item of annotations) {
      if (item.status !== 'open' || !touched.has(item.seg)) continue
  ...
  route('stage', async (body) => {
    ...
    // 起草是花时间的模型调用，结果会归档到 `<state>/阶段原始输出/<id>.md`。
    // 切页签会把界面上的草稿弄丢，所以给一个"读回上次起草"的入口（产物本来就在磁盘上）。
    if (action === 'recall') {
      const recallStage = body.stage === undefined ? status.current : String(body.stage)
      const target = PIPE.STAGES.find((x) => x.id === recallStage)
      if (target === undefined) throw new Error('未知阶段：' + String(recallStage))
      const archive = join(W.stateDirOf(project.projectDir), '阶段原始输出', `${recallStage}.md`)
      const text = await readTextSafe(archive)
      if (text.trim() === '') throw new Error(`还没有「${target.name}」阶段的起草记录（${archive}）`)
      // 归档文件带三行说明头，正文从第一个空行之后开始
      const bodyStart = text.indexOf('\n\n', text.indexOf('\n\n') + 1)
      return {
        stage: recallStage,
        text: bodyStart === -1 ? text : text.slice(bodyStart).trim(),
        archive,
    ...
    if (action === 'save') {
      ...
      // chapter 会进文件路径：必须是 1 起的整数。这里曾经漏了校验，是全文件唯一的
      // 越界写点（chapter 传 '../../…' 可以写到作品目录之外，审查实测复现）。
      const saveChapter = Number(body.chapter ?? rows.length + 1)
      if (!Number.isInteger(saveChapter) || saveChapter < 1 || saveChapter > 1e6) {
        throw new Error('chapter 必须是从 1 开始的整数')
      }
      const target = stage.produce.includes('{chapter}')
        ? join(project.projectDir, stage.produce.replace('{chapter}', String(saveChapter)))
  ...
  /**
   * 从两个候选池里选"作者选段更多"的那个（本章自己的 / 全局当前目录），并给出选段。
   * merge 与 status 共用这一套规则——否则会出现"预览说没有选中段，点合并却合出来了"。
   */
  const picksBestOf = async (ownDir, currentDir, chapter) => {
    const pools = [...new Set([ownDir, currentDir].filter((d) => typeof d === 'string' && d !== ''))]
    let best = []
  ...
  route('newchapter', async (body) => {
    ...
    if (action === 'status') {
      ...
      return {
        ...
        picks: await picksBestOf(paths.candidatesDir, dirOf(), chapter),
      }
    }
    ...
    if (action === 'finalize') {
      // 正文必须是真的字符串：传对象进来会写出 "[object Object]" 这种脏稿
      if (typeof body.text !== 'string') {
        throw new Error('要写入的正文必须是字符串（收到的是 ' + (body.text === null ? 'null' : typeof body.text) + '）')
      }
  ...
  // 取用原因：由宿主自己解析"这一段属于哪个候选池"，而不是让客户端传路径
  // （客户端传目录等于开一个任意写入口）。合并表里改原因就走这里。
  route('pick-reason', async (body) => {
    const project = await projectOf(false)
    if (project === null) throw new Error('还没认出作品目录')
    const chapter = chapterArg(body.chapter)
    const file = String(body.file ?? '')
    if (file === '') throw new Error('要指定候选稿文件名')
    const index = Number(body.index)
    if (!Number.isInteger(index) || index < 0) throw new Error('要指定段落序号')
    const paths = await newChapterPaths(project, chapter)
    const pools = [...new Set([paths.candidatesDir, dirOf()].filter((d) => typeof d === 'string' && d !== ''))]
    let hitDir = ''
    for (const pool of pools) {
      const found = await readCandidates(pool)
      if (found.some((c) => c.file === file)) {
        hitDir = pool
        break
      }
    }
    if (hitDir === '') throw new Error('这个候选稿不在本章的候选目录里：' + file)
    const reason = String(body.reason ?? '').trim()
    const key = `${file}#${index}`
    await updateJsonFile(join(stateDirOf(hitDir), REASONS_FILE), (reasons) => {
      if (reason === '') delete reasons[key]
      else reasons[key] = clip(reason, 200)
      return reasons
    })
    const reasons = await readReasons(hitDir)
    return { dir: hitDir, reasons, key }
  })
