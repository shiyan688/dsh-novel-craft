...
import { charCount, clip, readJsonSafe, splitSegments } from './text.js'
...
export async function addAnnotation(projectDir, chapter, input) {
  ...
  const written = await writeList(projectDir, chapter, list)
  return { added: existing === undefined, list: written.list, path: written.path }
}
...
export async function updateAnnotation(projectDir, chapter, id, patch) {
  ...
  if (target === undefined) throw new Error('没有这条批注：' + String(id))
...
export async function removeAnnotation(projectDir, chapter, id) {
  ...
  if (next.length === 0) {
    await unlink(annotationPathOf(projectDir, chapter)).catch(() => {})
    return { list: [], path: annotationPathOf(projectDir, chapter) }
  }
  const written = await writeList(projectDir, chapter, next)
  return { list: written.list, path: written.path }
}
